...
class Adam(StepStrategy):
    ...
    def updates(self, parms, grads):
        ...
        return list(zip([bm_t, bv_t], [new_bm_t, new_bv_t])) + \
        list(zip(acc_m, new_acc_m)) + list(zip(acc_v, new_acc_v)) + list(zip(parms, new_parms))


class DPSGD(StepStrategy): #! 
    """Adam step size strategy. For details, see:
    D. Kingma and J. Ba, "Adam: A Method for Stochastic Optimization", ICLR, 2015."""

    def __init__(self, max_grad_norm, noise_multiplier):
        """Constructor. Sets adam's hyperparameters."""
        assert max_grad_norm > 0, 'max_grad_norm must be positive.'
        assert noise_multiplier > 0, 'max_grad_norm must be positive.'
